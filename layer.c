...
void	layer_event(t_guimp *guimp)
{
	...
	// Goes through all the layer's buttons, to select which layer is the selected one;
	ii = -1;
...
void	layer_draw(t_guimp *guimp)
{
	...
	// NOTE: We want these here (layer_draw) so that we dont have to calculate some things twice (aspects);
	...
	*/
	...
	SDL_RenderCopy(guimp->win_main->renderer, guimp->final_image_texture, NULL,
		&(SDL_Rect){guimp->final_image.pos.x, guimp->final_image.pos.y,
			guimp->final_image.pos.w * guimp->zoom, guimp->final_image.pos.h * guimp->zoom});
	...
	SDL_RenderCopy(guimp->win_main->renderer, guimp->hidden_texture, NULL,
		&(SDL_Rect){0, 0,
			guimp->hidden_surface->w * guimp->zoom, guimp->hidden_surface->h * guimp->zoom}); 
